...
KV.prototype.Call = function Call(method, request, callback) {
	if(! request.get('header.user')) {
		request.set('header.user', this.user)	
	}
	if(this.user_priority != 0 && ! request.get('header.user_priority')) {
		request.set('header.user_priority', this.user_priority)
	}
	...
	this.sender.Send(call, function(err, res) {
		...
		var error = res.get('header.error')
		
		if(error) {
			console.log('returnng error from header')
			callback(error, res)
			return
		}
...
KV.prototype.Flush = function Flush(callback) {
	...
	for(var i in this.prepareBuffer) {
		batch[i] = this.prepareBuffer[i].Request
		cbs[i]   = this.prepareBuffer[i].Callback
	}
	...
	// Make call
	this.Call(proto.Batch, new proto.BatchRequest(batch), function FlushCallback(err, res) {
		if(err) {
			callback(err)
			return
		}	
...
KV.prototype.RunTransaction = function RunTransaction(opts, retryable, callback) {
	...
	txnKV.user_priority = this.user_priority
