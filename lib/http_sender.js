...
var HttpSendError = HTTPSender.SendError = function SendError(err) {
	this.__proto__ = new Error('error while processing send response (' + err.message + ')')
} 

// HTTPRetryOptions sets the retry options for handling retryable
// HTTP errors and connection I/O errors.
var HTTPRetryOptions = new retry.Options({
	backoff:     50, // ms
	maxBackoff:  5 * 1000,
	constant:    2,
...
function HTTPSender(opts) { 
	...
	this.http   = opts.http || require('http') // The HTTP client
}
...
HTTPSender.prototype.Send = function Send(call, callback) {
	...
	retry.RetryWithBackoff(retryOpts, function RetryFunction(retryCallback) {
...
HTTPSender.prototype.post = function(call, callback) {
	...
	var body = call.Request.encodeNB()
	...
	var req = this.http.request(opts, function(res) {
		...
		res.on('end', function() {
			try {
				...
				var response = builder.decode(buffer)
			} catch(e) {
				callback(
					new HttpSendError(e),
					null
