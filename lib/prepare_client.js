...
Client.prototype.conditionalPut = function ConditionalPut(key, value, expectedValue, callback) {
	this.kv.Prepare(proto.ConditionalPut, proto.ConditionalPutRequest(key, value, expectedValue), callback)
}

// Increment
Client.prototype.increment = function Increment(key, value, callback) {
	this.kv.Prepare(proto.Increment, proto.IncrementRequest(key, value), callback)
}

// Delete 
Client.prototype.delete = function Delete(key, callback) {
	this.kv.Prepare(proto.Delete, proto.DeleteRequest(key), callback)
}

// Delete Range 
Client.prototype.deleteRange = function DeleteRange(start_key, end_key, limit, callback) {
	this.kv.Prepare(proto.DeleteRange, proto.DeleteRangeRequest(start_key, end_key, limit), callback)
}

// Scan 
Client.prototype.scan = function Scan(key_start, key_end, limit, callback) {
	this.kv.Prepare(proto.Scan, proto.ScanRequest(key_start, key_end, limit), callback)
}

// Contains 
Client.prototype.contains = function Contains(key, callback) {
	this.kv.Prepare(proto.Contains, proto.ContainsRequest(key), callback)
}
