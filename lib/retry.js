...
Retry.prototype.RetryMaxAttemptsError = function RetryMaxAttemptsError(max) {
...
function RetryOptions(opts) { 
	...
	this.Constant    = opts.constant|| 0.0
...
// returns an error.
Retry.WithBackoff(options, retryFnc, callback) {
	...
	var loop = function loop() {
		...
		retryFnc(function(err, _status) {
			...
			if(_status === RetryReset) {
				...
				wait = Math.floor(backoff + Math.random() * (backoff / 1000 / 1000) * retryJitter)
				// Increase backoff for next iteration.
				backoff = Math.floor(time.Duration(backoff * opts.Constant))
